...
const gitcoinGrantsRoundsRefs: RoundRef[] = [
...
async function fetchApplications(refs: RoundRef[]): Promise<Application[]> {
  ...
  const applicationsWithFeatures = applications
    .sort((a, b) =>
      a.metadata.application.project.title.localeCompare(
        b.metadata.application.project.title,
      ),
    )
    .map(async (app, _i, total) => {
      const key = `application:${app.chainId}:${app.roundId}:${app.id}:features`;
      let features: Features | null = await cache.get(key);
      if (!features) {
        features = await throttledExtractFeaturesFromApplication(app);
  ...
  return await Promise.all(applicationsWithFeatures).then((apps) =>
    apps.flat(),
  );
}
...
export async function fetchAllApplicationsWithFeatures() {
  const applications = await fetchApplications(gitcoinGrantsRoundsRefs);
  return applications;
}
